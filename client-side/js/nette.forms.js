 ...
 *
 * @author Radek Ježdík, MartyIX, David Grudl, Martin Adamek
 */

var LiveForm = {
	options: {
		controlErrorClass: 'form-control-error',		// CSS class for an invalid control
		errorMessageClass: 'info-error help-inline',	// CSS class for an error message
		hintMessageClass: 'info-help help-inline',		// CSS class for a valid message
		validMessageClass: 'info-success help-inline',	// CSS class for a valid message
...
LiveForm.setUpHandlers = function(el) {
	...
	// does pair element exists? if not, do not use live validation
	if (document.getElementById(el.id + '-pair') === null) return;
	...
	if ($(el).hasClass('tags')) {
		var text = $(el).data('help-text');
		var id = $(el).attr('id');
		$(el).attr('id', id + '_hidden');
		setTimeout(function() {
			el = $(el).parent().find('ul input[type=text]')[0];
			if (typeof text !== 'undefined') {
				$(el).data('help-text', text);
				$(el).attr('id', id);
				$(el).off('focus.validation').on('focus.validation', function(e) {
					if ($(this).parent().find('.help-line').length === 0) {
						var error = self.getMessageElement(this);
						error.innerHTML = text;
						error.className = self.options.hintMessageClass;
					}
				});
			}
		}, 100);
	...
	$(el).off('change.validation, blur.validation').on('change.validation, blur.validation', handler);
	$(el).off('focus.validation').on('focus.validation', function(e) {
		var text = $(this).data('help-text');
		if (typeof text !== 'undefined' && $(this).parent().find('.help-line').length === 0) {
			var error = self.getMessageElement(this);
			error.innerHTML = text;
			error.className = self.options.hintMessageClass;
		}
	});
	$(el).off('keydown.validation').on('keydown.validation', function(event) {
	...
	$(el).off('keyup.validation').on('keyup.validation', function(event) {
		event = event || window.event;
		if (event.keyCode !== 9) {
...
//		el.parentNode.appendChild(error);
		...
		if (blockControl) {
			var parent = $(blockControl).parent();
			var controls = $(blockControl).find('.controls');
			if (!parent.is('.half')) {
				controls.append(error);
			} else if (parent.index() === 1) {
				controls.find('.help-inline').remove();
				if (!controls.find('.info-error.help-inline').length) {
					controls.append(error);
				}
			} else {
				controls = parent.parent().find('.half:eq(1) .controls');
				if (!controls.find('.info-error.help-inline').length) {
					controls.find('.help-inline').remove();
					controls.append(error);
				}
